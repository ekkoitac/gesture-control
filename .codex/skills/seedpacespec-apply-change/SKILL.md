...
description: >
  ...
  Codex 触发语义：使用 $seedpacespec-apply-change；兼容原 sdx-apply 工作流意图。
---
...
**写代码**实现 seedpacespec 变更中的任务。本 skill 的核心是编码实现，不是改设计——改设计请用 `$seedpacespec-update-task`。
...
| # | 状态 | 完成条件 | 未完成时行为 |
|---|------|---------|-------------|
| G1 | `change_selected` | 变更已确定（用户在本回合明确指定，或通过用户确认/输入流程 选择） | `list --json` + 请求用户选择 |
...
| G5 | `mode_selected` | 用户选定实现模式（A/B/C）；B→仅改 tasks 后结束；C→直接结束 | 向用户询问模式 |
...
4. G5 可由用户直接说"继续 A"跳过 向用户确认/请求输入
5. G6 从第一条未完成任务开始
...
| 步骤 | 易犯错误 | 正确行为 |
|------|---------|---------|
| **G3 上下文消化** | 跳过 TRD 或未归纳 API 惯例即编码 | 必须读取 contextFiles + TRD 全文；有 `architecture.md` 则读（侧重契约），无则从邻居代码归纳请求惯例 |
| **G4 消化摘要** | 跳过摘要展示直接询问模式 | 必须向用户展示消化摘要证明已读 |
| **Phase C 确认模式** | 忽略 `apply.batchMode` 配置 | 必须先读取 config.yaml 决定确认行为 |
| **Phase C 确认本身** | 实现完就标 `[x]` 不等确认 | 必须通过用户确认/输入流程 获得用户确认 |
| **反馈处理判定** | AI 自行判定规模后直接改代码 | 必须先 请求用户确认改动规模 |
...
**输入**：可选指定变更名。若用户未明确写出变更名，**必须**运行 `seedpacespec list --json` 并用 请求用户选择，禁止仅凭对话历史推断。

### 步骤 1: 选择变更（→ G1）

若用户在本回合消息中明确写出变更名则使用。否则：
- 运行 `seedpacespec list --json` 获取所有 active 变更
- 若仅有一个：展示并 **向用户确认/请求输入** 确认
- 若有多个：**向用户确认/请求输入** 让用户选择
- **禁止**仅凭对话历史自动推断选定变更
...
**处理状态**：
- `state: "blocked"` → 展示说明，建议 `$seedpacespec-apply-change`。**流程终止于 G2。**
- `state: "all_done"` → 祝贺并建议归档。**流程终止于 G2。**
- 否则 → 继续推进 G3→G6

**跨变更冲突检测（G2 通过后）**：读取其他 active 变更的 design.md 文件树，比对路径交叉。发现冲突 → 向用户确认是否继续。
...
⛔ 约束 [GL5]: 若 design.md 文件树**完全缺失**，必须暂停并 向用户确认/请求输入。
...
- **选 A** → 进入步骤 6 实现循环
- **选 B** → 重规划 tasks.md（拆为骨架阶段+实现阶段），不编写业务代码，完成后提示再次执行 `$seedpacespec-apply-change`
- **选 C** → 不改动仓库，流程结束
...
- ≥2 个独立任务 → 向用户展示并确认并行方案，用户选择并行或串行
...
**性能微调**：不扩大任务范围前提下优化明显低效点；若改变对外行为 → 向用户确认。
...
⛔ 约束 [GL8]: 必须先 请求用户确认改动规模，不允许 AI 自行判定后直接修改。

| 规模 | 判断标准 | 处理方式 |
|------|---------|---------|
| **轻量修改** | 同时满足：①单章节局部 ②不改文件树 ③不波及已完成任务 | 向用户确认后在 apply 内直接处理 |
| **重大修改** | 任一：①≥2 章节 ②改文件树 ③波及已完成任务 ④改架构策略 | 引导 `$seedpacespec-update-task` |
| **无法确定** | 不能明确归入上述 | 请求用户选择 |
...
- ⛔ **GL1** 状态机门禁（G1→G6）是 ASSERTION：任何状态未完成 → 只能向用户确认/请求输入，不得跳入后续状态。禁止跳过、合并或替用户做决定。禁止编造跳过理由。→ 见 State Machine Gates
...
- ⛔ **GL5** 路径以已确认文件树为第一约束：`trd.md`、`design.md` 树状图是文件路径唯一真源。文件树缺失时必须向用户确认/请求输入。→ 见步骤 3
- ⛔ **GL6** UI render 禁止自推断设计参考：必须向用户确认/请求输入 确认截图/JSON 是否存在；不得据 Figma 修改 TRD 已确认组件树。→ 见步骤 6.1
- ⛔ **GL7** 所有修改必须同步设计产物：轻量修改在 apply 内完成（含 TRD/design 更新）；重大修改引导 `$seedpacespec-update-task`。→ 见步骤 6.4
- ⛔ **GL8** 设计修改必须先 向用户确认规模：用户说"要改"或 AI 发现问题时，一律先展示影响范围让用户选择。拿不准时一律 向用户确认/请求输入。→ 见步骤 6.4
...
- `.codex/skills/seedpacespec-render-ui/SKILL.md` — UI 视觉还原规范（ui_render slot 默认实现）
- `.codex/skills/seedpacespec-update-task/SKILL.md` — 重大修改时的设计产物更新流程
