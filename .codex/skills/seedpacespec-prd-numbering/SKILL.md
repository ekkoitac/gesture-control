...
description: >
  ...
  examples: "$seedpacespec-prd-numbering specs/foo-prd.md" → 触发；"R01 是什么" → 不触发。
  Codex 触发语义：使用 $seedpacespec-prd-numbering；兼容原 sdx-prd-numbering 工作流意图。
---
...
本 skill **只做编号与结构对齐**，不扩写业务需求；若原文条目语义不清，请求用户确认后再写入。

---

## State Machine Gates

⛔ 约束：门禁未完成 → 只能向用户确认/请求输入，不得覆盖写入 PRD。

| #  | 状态                    | 完成条件                                               | 未完成时行为        |
|----|-------------------------|--------------------------------------------------------|---------------------|
| G1 | `prd_source_resolved`   | 已确定要处理的 PRD 文件路径（或变更名 + specs 内文件） | 请求用户选 |
| G2 | `strategy_confirmed`    | 用户确认编号策略（见下文「编号策略」）                 | 向用户确认/请求输入         |
...
在 G2 用 **Codex 用户确认/输入流程** 让用户选一项（可加一行说明默认推荐）：

1. **顺序重编（推荐）**：按文档中出现顺序（或「需求清单」表格行序）依次分配 `R01`、`R02`…，忽略旧编号冲突。
2. **保留原序号**：若原文已有稳定编号（如业务方的 REQ-5），只做格式归一 → `R05`（不足两位补零），**缺失的号段保留空缺或 向用户确认/请求输入 是否压缩连续**。
...
1. 若用户给了路径 → 读取该文件，校验为 Markdown。
2. 若用户给了变更名 → 在 `seedpacespec/changes/active-change/<name>/specs/` 下列出 `*.md`，向用户确认/请求输入 选一文件。
3. 若都没有 → 向用户确认/请求输入：粘贴路径 / 变更名 / 从 `seedpacespec list --json` 选变更。
...
- 枚举：`1.`、`（1）`、`- 需求：` 等（谨慎合并：同一标题下多个 bullet 可能是一条或多条，按语义向用户确认/请求输入）

列出 **候选条目清单**（标题摘要 + 原文位置线索），准备映射到 `R01…`。

### Step 3：编号策略与对照表（→ G2/G3）

1. 执行 **编号策略** 向用户确认/请求输入（见上）。
...
向用户确认/请求输入：**「以上编号是否确认？有需要合并/拆分的条目请说明。」**
...
- 处理文件路径
- 共几条需求（`R01`–`Rnn`）
- 下一步：可执行 `$seedpacespec-propose` 或 `$seedpacespec-trd-generator`（若变更目录已齐）
