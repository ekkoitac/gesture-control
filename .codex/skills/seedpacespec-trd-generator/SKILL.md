...
description: >
  ...
  Codex 触发语义：使用 $seedpacespec-trd-generator；兼容原 sdx-trd-generator 工作流意图。
---
...
生成 TRD 前必须依次通过以下状态门禁。**任何状态未完成 → 只能向用户确认/请求输入，不得进入生成。**

| # | 状态 | 完成条件 | 可验证产出 | 未完成时行为 |
|---|------|---------|-----------|-------------|
| G0 | `tech_stack_role_resolved` | **Step 2** 已确定本 spec 的技术栈与角色（写入 `seedpacespec/role.yaml`），并已选定要加载的 `references/<stack>/loaders.yaml` | role.yaml 文件 | 执行 Step 2，必要时 向用户确认/请求输入 |
| G1 | `mode_selected` | Step 4 用户选定模式（A/B/C/D） | 用户回复 | 向用户询问模式 |
| G2 | `r_scope_normalized` | 所有纳入的 R 已识别并归一化 | R 列表 | 向用户确认 R 范围 |
| G3 | `user_confirmed_scope` | 用户明确确认 R 范围和 TRD 文件名 | 用户回复 | 向用户展示并确认 R 列表确认 |
| G3.1 | `subpkg_confirmed` | monorepo：Q4 已展示全部子包并获用户确认；单包：自动通过 | 向用户确认/请求输入 含全部子包列表 | 执行 §4.2 |
...
| G4.1 | `extra_context_collected` | Step 5 向用户确认/请求输入 已发出且用户已回复 | 向用户确认/请求输入 消息 | 执行 Step 5 |
| G4.2 | `common_module_assessed` | Step 7 公共模块抽离判定已输出结论（"无需抽离"或抽离建议的 向用户确认/请求输入） | 判定结论文本 | 执行 Step 7，输出判定 |
| G5 | `references_loaded` | **Step 8 评估表已输出**（条件来自 G0 选定的 `loaders.yaml`），且所有"命中=✅"行的文件已通过 文件读取操作读取（"已读取"列为 ✅ 或 —） | 评估表 | 执行 Step 8，输出评估表 |
...
`读取文件 seedpacespec/role.yaml`——若文件存在且结构有效 → **直接采用**，跳到 2.6。
...
6. 以上均不存在时 → fallback 到内置路径：`读取文件 .codex/skills/seedpacespec-trd-generator/references/loaders.yaml`
...
2. **Q1 子包候选确认**（naive 信号必问；tooling 可跳过）：列出候选子包 → 请求用户确认/调整
...
- **优先源 `architecture.md`**：读取文件 → 用 `architecture_md_hints`（keywords / categories / directory_signals）匹配 → 输出命中清单。若 architecture.md 中标注了 `项目形态: sdk` → 优先匹配 sdk 技术栈。
...
| 命中情况 | 行为 |
|---------|------|
| **唯一命中** | 检查 `requires_role_disambiguation`：`false` → 自动采用并告知；`true` → **Q2 向用户确认/请求输入** 消岐（典型：后台信号 → 真后台 / BFF / 两者兼有） |
| **多个命中** | 按 `multi_stack_fallback` 请求用户选主角色 |
| **零命中** | 按 `on_unmatched` 向用户确认/请求输入 手选，提示团队补检测规则 |
...
- 单包：`读取文件 references/<role.loads>/loaders.yaml`
...
4. **加载反馈记忆**（必须执行）：按 `.codex/skills/_shared/memory-protocol.md` 的"读取时机"执行——读取 `seedpacespec/config.yaml` 的 `memory.enabled`，为 `true` 时读取 `seedpacespec/memory/team-feedback.md` 和 `personal-feedback.md`（若存在），作为生成约束。
5. **扫描项目已有代码模式**：找同类代码（Model / Repository / slice 等）→ 分析命名规范、文件组织、代码结构模式 → 后续 §4 必须与已有代码保持一致。
6. **design.md 机器标记**（若变更目录存在 `design.md`）：**读取文件** 全文或尾部，解析 HTML 注释：
...
#### §4.1 生成模式选择（G1 门禁 · Q3 向用户确认/请求输入）

| 模式 | 适用场景 | 需求选择方式 | 循环 |
|------|---------|-------------|------|
| **A. 全量生成** | 小项目/首次 | 自动包含所有 R，但仍须在 G3 展示完整 R 列表并 向用户确认 | ❌ |
...
#### §4.2 受影响子包确认（G3.1 门禁 · Q4 向用户确认/请求输入，monorepo 必须执行）

⛔ 约束: 单包跳过。monorepo 必须执行——即使"只影响一个子包"也必须列出全部子包让用户确认。任何模式（A/B/C/D）均不可跳过。不得将本步与 §4.3 合并为一次询问。

每条 R 进入生成前，基于 PRD 的模块名 / 路径 / 跨包调用提及，初步划入"受影响子包清单"，请求用户确认/调整（多选）。

⛔ 约束: 向用户确认/请求输入 必须列出全部已确认子包（AI 预勾选可能受影响的，但不得省略其他子包）。
...
#### §4.3 monorepo 生成节奏（Q5 向用户确认/请求输入，仅 §4.2 勾选 ≥2 时触发）
...
| **phased_with_confirm** | 每完成一份 向用户确认/请求输入 是否继续下一份 |
...
2. Q-next-subpkg（下一份子包）与 Q-next-req（下一条 R）是两个独立 向用户确认/请求输入，不能合并
...
#### §4.5 深度分级（G3.2 门禁 · Q-depth 向用户确认/请求输入）
...
- **与 propose `design.md` 联动（推荐倾向，不替代用户确认）**：若 Step 3 已解析到 `<!-- sdx:design-mode=lite -->`（即用户在 propose 中确认采用**精简 design**），则 Q-depth **默认倾向 Eval（影响评估）**——对每条 R，**拿不准时优先标 Eval 而非 Full**；评估表「推荐理由」中须点名「design 已标记 `sdx:design-mode=lite`，TRD 深度与之对齐」。**仍必须**输出本条 向用户确认/请求输入，用户可将任意 R 升为 Full 或降为 Min。
...
> AI 必须在 Step 2 技术栈判定后，读取对应栈的 `references/<stack>/loaders.yaml`，提取 `depth_dimensions` 数组作为本次可选维度菜单。若项目为 monorepo 且子包技术栈不同，各子包使用各自栈的维度清单。
  
1. **必须**输出深度分级评估表（向用户确认/请求输入），等待用户确认后才能继续。用户可对任意 R 升降级，或增删展开维度。格式如下：
...
### Step 5: 额外上下文（G4.1 门禁 · 向用户确认/请求输入，一次性收集）

⛔ 约束: 不得跳过。本步唯一合法输出 = `[GATE G4.1]` 标记 + 上下文收集消息 + AskUserQuestion，输出后立即停止。即使"没什么要问的"也必须发出 AskUserQuestion 让用户确认。

**用一次用户确认/输入流程 统一处理**，包含以下内容（合并为一条消息，不逐项追问）：

1. **缺失上下文告知**：列出 Step 3 中检测到缺失的可选上下文（C2/C4/C5/C6），说明将以"基于推断"方式处理，用户若有补充可在此提供路径
2. **额外关注点**：性能/安全/兼容等
3. **参考文件路径**：用户想额外纳入的文件
4. **特殊约束**
5. **（可选）对标 TRD 路径**：若用户提供仓库内已有 TRD（如 `trds/r06-xxx-trd.md`），本步一并收集路径；**生成 §4 时须读取 该文件**，章节厚度与写法**不得低于**该参考（薄 PRD 也不能比参考 TRD 更「提纲化」）
...
扫描 Step 6 的改动方案，若出现以下情况 → 用 向用户确认/请求输入 建议先抽离：
...
4. 公共 TRD 保存后，**向用户确认下一步**：
   ...
   - 用户选 B → 根据用户反馈修改公共 TRD → 保存 → 重新回到本用户确认流程（用户可继续选 B 反复修改，直到满意后选 A 继续）
...
2. **必读项**：把 `loaders.yaml.mandatory` 列表里的所有文件全部读取（典型至少包含黄金样本）
...
5. 对所有"命中=✅"的行，**立即用 文件读取操作读取对应文件**（路径以 loaders.yaml 中 `file` 字段为准，相对其所在目录解析）
...
⛔ **无专属 guide 时的强制兜底**：此时**必须额外读取 `references/stack-thinking-skeleton.md`**，重点阅读其中对应技术栈的「各层核心设计问题」表和「设计意图必答题」
...
| 输入 | 角色 | 说明 |
|------|------|------|
| **骨架** `templates/trd-skeleton.md` | 结构框架 | 决定 TRD 有哪些章节、章节顺序、章节间约束关系。骨架中 blockquote 段落（architecture/design/TRD 三者关系、分层来源规则、写作规则等）是**强制生成约束**，必须逐条遵守。⛔ 生成前必须已读取 |
...
若 Step 5 用户提供了**对标 TRD 路径**：生成前 **读取文件** 该文件；最终 §4 的展开深度**不得低于**该参考。

**外层循环边界**：本步每次执行生成**一份完整 TRD**（当前 R × 当前子包）。生成完毕后进入 Step 9.5 确认循环。外层循环由 §4.4 控制（模式 C/D 循环多条 R；monorepo 循环多个子包）。

**内层分块（单份 TRD 内部的生成策略）**：

> 用途：当单份 TRD 复杂度较高时，需要在生成过程中分块输出 + 中间确认，避免一次性输出过长导致方向偏离。分块生成的具体 向用户确认/请求输入 流程由 `references/<stack>/advanced-scenarios.md` 定义。
...
| **边界** | 恰好 5 R 或恰好 3 层 | 向用户确认/请求输入：「本模块包含 N 个需求 / M 个架构层，建议分块生成。」选项：A）分块生成；B）直接全量 |
| **多模板** | 多 R 模板组 / 单 R 内多模板 | 读 `advanced-scenarios.md` §多模板，按其中定义的 用户确认模板确认流程执行 |
...
3. 达到任一上限后：必须停止继续重写/重生成，改为 向用户确认/请求输入 一次性收集“失败原因 + 需要补充的上下文/约束”（如：具体不满意点、必须遵守的接口/字段、参考代码路径、期望的章节深度、必须/禁止的技术选型），并明确告知“收集到信息后再进入 Step 9 重写”
...
2. **反馈记忆收集**（必须执行）：按 `.codex/skills/_shared/memory-protocol.md` 的"收集时机"执行——分析本轮确认过程是否有值得记录的纠正/肯定/决策信号，有则写入记忆文件并简短告知用户。
...
⛔ **Full 级别生成 TRD 前，必须先读取 骨架模板**：

```
读取文件: <本 skill 目录>/templates/trd-skeleton.md
```
