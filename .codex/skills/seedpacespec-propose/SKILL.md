...
description: >
  ...
  examples: "$seedpacespec-propose 用户登录功能" → 触发；"我想加个登录" → 仅在 sdx 会话上下文中触发。
  Codex 触发语义：使用 $seedpacespec-propose；兼容原 sdx-propose 工作流意图。
---
...
**输入**：`$seedpacespec-propose` 后的参数为变更名（kebab-case），或对要做内容的描述，或不提供参数。

---

## State Machine Gates

⛔ 约束 [GL2]: 任何门禁未完成 → 只能向用户确认/请求输入，不得进入后续步骤。状态推进单向 G1→G9，不得跳跃。

| #  | 状态                    | 完成条件                                                                 | 未完成时行为                     |
|----|-------------------------|--------------------------------------------------------------------------|----------------------------------|
| G1 | `input_understood`      | 用户已选择已有变更或新建变更，kebab-case 名称已确定                      | 向用户确认/请求输入 列出变更或询问新建   |
| G2 | `architecture_resolved` | architecture.md 存在，或用户选择补充/跳过                                | 向用户询问是否补充         |
| G3 | `change_created`        | `seedpacespec new change` 已执行，变更目录存在                           | 执行 CLI 创建目录                |
| G4 | `prd_resolved`          | PRD 已落在变更目录 `specs/`，或用户确认现场生成/已提供路径/明确跳过并记风险 | 向用户确认/请求输入 收集 PRD 来源        |
| G5 | `design_mode_selected`  | 用户已通过用户确认/输入流程 确认 design.md 使用完整模式或精简模式             | 向用户确认/请求输入 推荐并让用户选择     |
| G6 | `schema_artifacts_done` | proposal + design（含已确认文件树）已创建                                | 按依赖序生成产物                 |
| G7 | `trd_confirmed`         | TRD 已生成且用户确认不需修改（确认循环通过）                             | 向用户确认循环             |
...
⛔ 约束 [GL5]: 恢复协议不可跳过。当变更目录已存在时，必须向用户确认/请求输入 询问意图，不得自行推断或替用户决定。
...
- 进入阶段 5.4：调用 **TRD generator skill**（推荐模式 C 或 D），生成新 R 的 TRD
  - ⛔ 约束: **“TRD 生成模式”专指 `.codex/skills/seedpacespec-trd-generator` 的 Step 4 §4.1（Q3）里让用户选择的生成模式（A/B/C/D）**，必须由 trd-generator 发起 请求用户选定；`seedpacespec-propose` 禁止替用户代选/推断模式
...
⛔ 约束: 推断出断点 gate 后，必须向用户确认/请求输入 展示推断结果并确认：「检测到已有产物 [列表]，推断从 G<N> 继续。是否从此处恢复？」禁止静默跳跃。
...
- 用户选择「新建变更」→ 向用户确认/请求输入 开放式提问「请描述你想构建或修复的内容」→ 推导 kebab-case 名称 → **向用户确认**名称
...
- **已存在**：进入下一步
- **不存在，但项目已有代码**：向用户确认/请求输入「是否要补充 architecture.md？」→ 选是则执行 `$seedpacespec-analyzer-project`；选否则记录风险后继续
- **不存在，且为全新项目（无已有代码或仅有脚手架）**：请求用户选择：
...
按 `.codex/skills/_shared/memory-protocol.md` 的"读取时机"执行：读取 `seedpacespec/config.yaml` 的 `memory.enabled`，为 `true` 时读取记忆文件作为后续生成约束。
...
2. 输出下一步指引（示例）：
   - 执行 **`$seedpacespec-explore`**；若当前已有变更名 **`{name}`**（阶段 1 已确定且阶段 3 可能已创建目录），建议在指令或首轮对话中带上该名称，便于 PRD 落入 `seedpacespec/changes/active-change/{name}/specs/`。
   - explore 完成澄清并写入 PRD 后，再执行 **`$seedpacespec-propose {name}`**（或用户改用 guided）继续生成 design/TRD/tasks。
...
在 PRD 文件已落入 `specs/` 且可用 **读取文件** 读取全文后（「暂不提供 PRD」跳过本节）：

1. **快速检测**（读取全文后执行）：  
   - **标准形态**：`\bR\d{2}\b`（如 `R01`、`R09`）。  
   - **常见非标准但仍算「有 ID」**：`\bR-\d{2}\b`（如 `R-01`，下游归一化时会等价处理）。  
   - **两者皆无**：视为 **缺少可识别的 R 需求编号**，**必须向用户确认/请求输入**，不得静默进入阶段 5。  
   - **仅有 `R-01` 类而无 `R01` 类**：**建议** 向用户确认/请求输入 是否执行 `$seedpacespec-prd-numbering` 统一为 `R01` 正文写法（非强制）。  
   - **已有多处 `R01`/`R02`…**：可进入阶段 5；若明显缺号、跳号或与小节无法对应，**建议** 向用户确认/请求输入 是否归一化（非强制）。

2. **向用户确认/请求输入（在「标准形态与非标准形态均未检出」时强制）**  
   - question: "当前 PRD 中未检测到标准的 `R01` / `R02` … 需求编号。下游 TRD、tasks 通常依赖该编号对齐。请选择："  
   - option 1: label="现在归一化" description="执行 `$seedpacespec-prd-numbering`，按 seedpacespec-prd-numbering skill 补齐编号（写回或另存）"  
...
3. 用户选「现在归一化」→ **必须**加载并遵循 `.codex/skills/seedpacespec-prd-numbering/SKILL.md` 全文执行，完成后再回到阶段 5。
...
**执行主体**：由**大模型在给出模式推荐之前自行完成**下列两步推断——**不需要**用户填写「步骤 1 / 步骤 2」的单独表单；对用户仍只发起 **一次**文末「精简 / 完整」向用户确认/请求输入。
...
**合并进用户可见推理**：发起模式 向用户确认/请求输入 前，推荐理由须含 **各一句**——**步骤 1（体量，依据 PRD）**、**步骤 2（落点与是否在历史架构上演化，或为何弃用历史架构）**。
...
给出**推荐模式**及**简短推荐理由**（含上述推断要点），然后 **向用户确认/请求输入** 让用户最终决定（用户可不采纳推荐）：
...
1. **文件树确认**：写入树状图后 向用户确认/请求输入「以上为拟定的模块与文件结构，是否确认？」确认后标注「（已与用户确认）」
2. **整体设计确认**：向用户确认/请求输入「当前设计方案是否认可？」
...
**若经恢复协议选项 B 进入本步**：按恢复协议「选项 B：追加新模块」的规则执行——此处的“TRD 生成模式”指 `seedpacespec-trd-generator` 的 Step 4 §4.1（Q3）生成模式选择；由 trd-generator 负责 请求用户选定，新 TRD 写入 `trds/` 不覆盖已有文件。
...
🚀 下一步: 执行 $seedpacespec-apply-change {name} 开始实现
```
...
- ⛔ **GL1** 禁止业务代码生成：本工作流（第一阶段）**不得**创建、修改或删除业务仓库中的应用实现代码（`src/`、`app/`、`packages/` 等目录下的源文件）。即使用户明确要求「顺手实现」「先写一版代码」，也必须拒绝并说明须使用 `$seedpacespec-apply-change`。允许写入的范围仅限：seedpacespec 变更目录内的 `.md` 产物、根目录 `architecture.md`、运行 `seedpacespec` CLI。design.md 中的伪代码、树状图、接口说明等文档表述不视为业务代码。
- ⛔ **GL2** 状态机门禁（G1→G9）是 ASSERTION：任何状态未完成 → 只能向用户确认/请求输入，不得进入后续步骤。凡需要用户交互的步骤，必须停下等用户明确回复。禁止跳过、合并或替用户做决定。→ 见 State Machine Gates 章节
- ⛔ **GL3** design 模式（完整/精简）必须经用户 向用户确认后才能生成，不得 AI 单方面决定。→ 见阶段 5.3
- ⛔ **GL4** design 文件树未经用户确认，不得视为最终结构；须在 TRD 生成前完成确认。→ 见阶段 5.3 评审流程
- ⛔ **GL5** TRD 确认循环：TRD 每次更新后必须重新询问用户确认，直到用户明确表示不需修改。→ 见阶段 5.5
- ⛔ **GL6** 同名变更已存在 → 强制 向用户确认/请求输入 问意图（A/B/C），不得自行推断。选项 B 下不覆盖已有产物；选项 A 需二次确认覆盖。→ 见恢复协议章节
- ⛔ **GL7** 上下文不清时按类型区分处理：
  - 需求范围不清 → 以 PRD 为准；PRD 未覆盖的才 向用户确认/请求输入
  - 技术细节不清 → 向用户确认/请求输入 一次性确认：「以下技术细节暂不可用：`<列表>`。是否允许基于合理假设先行设计？」用户同意后，统一标注 `[假设]`，不再逐个追问
...
- ⛔ **GL11** 已有 PRD 但全文既无 `\bR\d{2}\b` 也无 `\bR-\d{2}\b` 时，必须先完成阶段 4「PRD 需求编号检查」的 向用户确认/请求输入（或完成归一化），不得直接进入阶段 5。→ 见阶段 4
...
- `.codex/skills/_shared/memory-protocol.md` — 反馈记忆读取/收集协议
- `.codex/skills/seedpacespec-trd-generator/SKILL.md` — TRD 生成器
